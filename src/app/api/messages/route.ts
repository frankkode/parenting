...
export async function POST(request: NextRequest) {
  try {
    ...
    const { familyCaseId, recipientId, recipientIds, subject, content, type } = body;
    ...
    // Support sending to multiple recipients (e.g., both parents)
    const recipientList: (string | null)[] = recipientIds
      ? recipientIds
      : [recipientId || null];

    const messages = [];
    for (const recId of recipientList) {
    ...
    return NextResponse.json(
      recipientIds ? messages : messages[0],
      { status: 201 }
    );
  } catch (error) {
